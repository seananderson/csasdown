...
fix_envs <- function(x,
  ...
  if (length(region_line) > 0) {
    ...
    pat <- "\\\\rdRegion\\}\\{(.*?)\\}+$"
    region_vec <- regmatches(x[region_line],
                             regexec(pat,
                                     x[region_line]))[[1]]
    ...
    if(fr()) {
      ...
      if(length(eng_match)){
        ...
        if(length(region_def_ind)){
          x[region_def_ind] <- gsub(region_supplied,
                                    region,
                                    x[region_def_ind])
        }
      }
    ...
    # Insert contact info
    x <- sub("AddressPlaceholder",
             contact_info$address,
             x)

    x <- sub("EmailPlaceholder",
      paste0("\\\\link\\{mailto:",
             contact_info$email,
             "\\}\\{",
             contact_info$email,
  ...
  for (i in seq(appendix_line + 1, length(x))) {
    ...
    if (!fr()) {
      x[i] <- gsub("\\\\chapter\\{",
                   "\\\\starredchapter\\{APPENDIX~\\\\thechapter. ",
                   x[i])
    } else {
      x[i] <- gsub("\\\\chapter\\{",
                   "\\\\starredchapter\\{ANNEXE~\\\\thechapter. ",
                   x[i])
    }
  }
  ...
  # \eqref needs to be \ref so the equation references don't have ()
  # around them: https://tex.stackexchange.com/a/107425
  x <- gsub("\\\\eqref\\{", "\\\\ref\\{", x)
  ...
  fig_labels <- gsub("\\\\caption\\{(.*?)\\}\\\\label\\{fig:(.*?)\\}",
                     "\\2",
                     figlabel_lines)
  all_include_graphics <- grep("(\\\\includegraphics\\[(.*?)\\]\\{(.*?)\\})",
                               x)
  ...
  if (identical(length(fig_labels), length(all_include_graphics))) {
    ...
    # nocov start
    alert("The number of detected figure captions did not match ",
          "the number of detected figures. Reverting to unnumbered ",
          "alternative text figures.")
  ...
  .matches <- lapply(regexs, \(.x){
    grep(.x, toupper(x), perl = TRUE) + 1})

  references_insertion_line <- unlist(.matches)

  x[references_insertion_line - 1] <- sub("chapter",
                                          "section",
                                          x[references_insertion_line - 1])
  x[references_insertion_line] <- sub("chapter",
                                      "section",
  ...
  if (length(references_insertion_line) > 0) {
    ...
    if (length(references_begin) > 0) {
      ...
      x <- c(
        ...
        paste0("\\setlength{\\parindent}{0in} ",
      ...
      starred_references_line <- grep(paste0("\\\\section\\*\\{REFERENCES\\}",
                                             "\\\\label\\{references\\}\\}"),
                                      x)
      if(length(starred_references_line)){
        x[starred_references_line] <- gsub("\\*",
                                           "",
                                           x[starred_references_line])
        # Remove the add contents line which was used to add the unnumbered section before
        add_toc_contents_line <- grep(paste0("\\\\addcontentsline\\{toc\\}\\",
                                             "{section\\}\\{REFERENCES\\}"),
                                      x)
      ...
      if(fix_ref_section_name){
        ...
        if(!length(ref_ind)){
          # nocov start
          bail("REFERENCES section header not found in the document. ",
               "Make sure you haven't commented out that section in ",
               fn_color("_bookdown.yml"), " or changed the header name")
          # nocov end
        }
        x[ref_ind] <- gsub("REFERENCES",
                           ifelse(fr(),
                                  "R\u00c9F\u00c9RENCES CIT\u00c9ES",
  ...
  for (i in seq_along(label_app)) {
    if (grepl("^\\\\section\\{", x[label_app[i] + 1])) {
      x[seq(label_app[i], label_app[i] + 1)] <-
        x[seq(label_app[i] + 1, label_app[i])]
    }
  }
  ...
  if (prepub) {
    # Text to add
    addText <- ifelse(fr(),
                      " -- \\hl{\\,Ne\\:pas\\:distribuer\\,}}",
                      " -- \\hl{\\,Not\\:for\\:further\\:distribution\\,}}")
    # 1. Modify header first page (report number)
    rn_loc_1 <- grep("\\% Report number", x) + 1
    rn_loc_2 <- grep("\\% End of report number", x) - 1
    ...
    st_loc_1 <- grep("\\% Title short", x) + 1
    st_loc_2 <- grep("\\% End of title short", x) - 1
    if (st_loc_1 != st_loc_2) {
      bail("Can't find short title (title_short)") # nocov
    }
    st_text <- x[st_loc_1]
    st_text_clean <- gsub("\\}+$", "", st_text)
    ...
    if (fr()) {
      ...
      cite_head_fr <- grep(
        paste0("La pr\\\\\'\\{e\\}sente publication ",
               "doit \\\\\\^\\{e\\}tre cit\\\\\'\\{e\\}e ",
               "comme suit~:"),
        x)
      if (length(cite_head_fr) == 0) {
        bail("Can't find French citation header") # nocov
      }
      x[cite_head_fr] <- "Cite comme ceci (jusqu'\u00E0 la publication)~:"
      cite_loc_fr <- grep(
        "\\\\citeFr\\{\\\\rdWorkDoneYear\\{\\}/\\\\rdNumber\\{\\}\\}",
        x = x)
      if (length(cite_loc_fr) == 0) {
        bail("Can't find French citation") # nocov
      }
      ...
      cite_head_eng <- grep(
        ...
        x)
      if (length(cite_head_eng) == 0) {
        bail("Can't find English citation header") # nocov
      }
      x[cite_head_eng] <- ""
      cite_loc_eng <- grep(
        "\\\\citeEng\\{\\\\rdWorkDoneYear\\{\\}/\\\\rdNumber\\{\\}\\}",
        x)
      if (length(cite_loc_eng) == 0) {
        bail("Can't find English citation") # nocov
      }
      x[cite_loc_eng] <- ""
    } else {
  ...
  x <- gsub("Res\\. Doc\\. ([0-9]{4}/[0-9]{2,}):",
            "Res. Doc. \\1.",
            x)
  x <- gsub("MPO\\. Doc\\. de rech ([0-9]{4}/[0-9]{2,}):",
            "MPO. Doc. de rech \\1.",
            x)

  # Pandoc now turns DOIs into href in references but must be \link{}
  # to have underline:
  x <- gsub("\\\\href\\{", "\\\\link\\{", x)
  ...
  # if (!pandoc_curr_ver_is_before()) {
  #   stop("csasdown currently only works with pandoc < 3.1.7."
  #        "Please revert to an older pandoc version.")
  # }
  ...
  if(length(theme_ind)){
    ...
    if(highlight %in% themes){
      theme_latex <- readLines(system.file("themes",
                                           paste0(highlight, ".latex"),
                                           package = "csasdown"))
    }else{
