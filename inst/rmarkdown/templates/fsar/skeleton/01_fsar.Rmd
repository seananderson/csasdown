...
g <- ggplot(data=sim.df, aes(x=year, y=ts.value)) +
  ...
  facet_wrap(vars(panel.category), scales="free") +
...
g <- ggplot(data=sim.df, aes(x=year, y=ts.value))
g1 <- g + geom_line(data=sim.df[sim.df$ts.name=="Catch-MT",]) + labs(x="Year", y="Catch-MT")

g2 <- g + geom_line(data=sim.df[sim.df$ts.name=="SSB-MT",]) + labs(x="Year", y="SSB-MT")

g3 <- g + geom_line(data=sim.df[sim.df$ts.name=="F-1/yr",]) + labs(x="Year", y="F-1/yr")

g4 <- g + geom_line(data=sim.df[sim.df$ts.name=="R-E06",]) + labs(x="Year", y="R-E06")
