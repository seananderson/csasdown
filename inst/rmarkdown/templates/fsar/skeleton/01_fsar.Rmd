...
Four-panel template using ggplot with `theme_csas()`:
...
```
And yet another take to see if geom_ribbon could work for showing uncertainty estimates:

```{r templates-3}
## alternative using cowplot

panel1.df <- tidyr::pivot_wider(sim.df[which(sim.df$panel.category=="SSB"),], names_from="ts.name", values_from="ts.value")
names(panel1.df) <- c("row.number","col.number","panel.category","year","SSB","SSBlow","SSBhigh","SSBlim")
