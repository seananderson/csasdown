...
Appendices can be in one file, or if they are larger than a couple of pages you should add a new file for each new appendix. In the first appendix, you **MUST** include two lines of code at the top to tell csasdown that you are now numbering sections as appendices. Look in `05_appendices.Rmd` to see these. The last line of your last appendix **MUST** be another line of code which tells csasdown to end the appendices sections. This can be found at the end of `05_appendices.Rmd`.

Figures and tables will now be prepended with the appendix letter:

(ref:test1) English verion of the test1 figure caption
...
Here's an equation. Note that it is automatically given a label on the right side of the page.
\begin{equation}
  1 + 1
  \label{eq:test2}
\end{equation}

Equations are referenced in the same way as figures and tables, but use the `eq:` tag instead of `fig:` or `tab:`. The difference is that you must include that tag when you write you equation (see code for above equation). For figures and tables, you don't. The full reference for this example equation is `\@ref(eq:test2)`.
...
To illustrate the new labelling of appendices, here are a table and figure:

(ref:test1b) English verion of the test1b figure caption
