...
The code above appears quite complex at first, but essentially the data file is read in, and each column is translated with some rules, and finally the column names are translated. The `rosettafish` package has an English-French dictionary for DFO nationwide that you can contribute to simply by going to [the GitHub site](https://github.com/pbs-assess/rosettafish) and editing the dictionary on the web.

When you choose how you want to present your table, you may (should) choose a simpler data format. This particular one has words which may or may not be inside parentheses in the `Source` column, and words which may be seperated by colons in the `Data` column. Those are all parsed out in the function which makes the code complex. The `Years` column is also complex because it contains the word "to", e.g. `1972 to 2019` which when translated to French becomes `De 1972 à 2019` which is difficult to write code for. If you chose to have two columns instead, `Start year` and `End year`, this function would be much simpler. The next code chunk does exactly this.
