...
(ref:testtab) Test figure with a caption will be numbered automatically.

(ref:testtab-french) French caption would go here once you've had it translated.
...
To reference the tables and figures, just place code like this for tables: `\@ref(tab:testtab)` inline in text (`testtab` is the knitr code chunk name) and like this for figures: `\@ref(fig:testfig)`. Note the difference in the tag preceeding the colon in those code snippets, `tab` for table references and `fig` for figure references. Here are the table references so far in this document: Table \@ref(tab:testtab) and Table \@ref(tab:testtab2), and the figure reference: Figure \@ref(fig:testfig). The numbers are clickable.
...
6. Removes all NA's from the grouping column, replacing them with empty strings
7. Make the table
8. The 5 `kableExtra::row_spec(XX, hline_after = TRUE)` commands - insert a horizontal line after each group. You need to look at your data and change these numbers, and probably add or remove some
9. The 2 `kableExtra::column_spec(XX, width = "XXem")` commands - change the width of the two columns to a set value. This has the effect of wrapping the text so it doesn't run off the page

Understanding how this table works will take you a long way in LaTex table generation. For example you could make any text bold that you want by using `\textbf{}` instead of `\emph{}`. To do that, you would just change the `emph` in the data file (`multirow.csv`) to `textbf` and add a new line in the code below `mutate_all(function(x){gsub("textbf\\{", "\\\\textbf\\{", x)}) %>%`.
