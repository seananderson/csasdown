...
Many error messages are in relation to the temporary files and state line numbers in those files, so it won't be long before you run the code to keep those files around. **Make sure to make your edits in the original files and not the temporary ones as the temporary ones are overwritten every time you re-render the document!**
