...
cat("## Inline R code inside cat()
Inline R code can be used in just the same way as in regular RMarkdown. For example we can find the name of a random car in the `mtcars` dataset: `r sample(rownames(mtcars), 1)` or get the date and time this document was built: `r as.character(Sys.time())`.")
```
