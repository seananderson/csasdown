...
test_that("csas_table() detects 0 year columns correctly", {
  ...
  expect_true(length(grep("1,000.000", x[[1]])) == TRUE)
  expect_true(!length(grep("1000.000", x[[1]])))
  expect_true(length(grep("1,001.000", x[[1]])) == TRUE)
  expect_true(!length(grep("1001.000", x[[1]])))
})

test_that("csas_table() detects 1 year column correctly", {
  x <- csasdown::csas_table(df2, format = "latex")
  expect_true(length(grep("2001", x[[1]])) == TRUE)
  expect_true(!length(grep("2,001", x[[1]])))
  expect_true(length(grep("2050", x[[1]])) == TRUE)
...
test_that("csas_table() detects 2 year columns correctly", {
  ...
  expect_true(length(grep("1901", x[[1]])) == TRUE)
  expect_true(!length(grep("1,901", x[[1]])))
  expect_true(length(grep("1950", x[[1]])) == TRUE)
...
test_that(paste0("csas_table() detects 2 year columns correctly and ignores ",
  ...
  expect_true(length(grep("40,001", x[[1]])) == TRUE)
  expect_true(!length(grep("40001", x[[1]])))
  expect_true(length(grep("40,050", x[[1]])) == TRUE)
...
test_that(paste0("csas_table() detects 2 year columns correctly makes exception ",
  ...
  expect_true(length(grep("40001", x[[1]])) == TRUE)
  expect_true(!length(grep("40,001", x[[1]])))
  expect_true(length(grep("40050", x[[1]])) == TRUE)
